...
{{ dt_duckietown_viewer_launch_tabs.format("Extrinsics Calibrator", "calibrate_extrinsics") }}
...
2. Position your Duckiebot on the calibration board such that it faces the checkerboard pattern, your Duckiebot's axle is parallel to and directly above the __large__ `y`-axis of the calibration board, and the axis that is perpendicular to and midway between your Duckiebot's axle is directly above the __large__ `x`-axis of the calibration board (if the `Check error` switch is set to `on`, the trapezoid in the image under `Camera View` should turn from black to blue).
...
```{include} ../../_includes/calibration/image-loading-trouble.md
```
