...
In some cases, you may have an option to run the actual demo code on your _laptop_ and communicate with the robot, or
to directly run the code on the _robot_.

```{tip}
In most cases, you should try to run the code natively on the robot since this will be more _reproducible_. When you run the code on your laptop you will be connecting to the robot through a wireless network which will induce latency and cause unexpected or poor performance.
```
...
the image will be _pulled_ from the registry, which can take some time.
```
...
To run the code on the laptop/base station you can switch the `-H`, which specifies the _machine_ to run the code on, with a `-R`, which specifies which _robot_ to connect to.
