...
### 1. __Clone the flasher repository:__
...
### 2. __Download the Linux4Tegra BSP and image:__
...
### 4. __Put the Jetson Nano into recovery mode:__
...
### 5. __Run the flashing container:__

   ```shell
   docker-compose run --rm ubuntu
   ```

### 6. __Inside the container, prepare and flash:__
...
Make sure to __remove the jumper__ shorting the `FC REC` and `GND` pins.
...
#### 1. __Clone the device tree overlays repository:__

   ```shell
   git clone https://github.com/duckietown/seeed-linux-dtoverlays.git
   ```

#### 2. __Configure and build the overlay:__
...
#### 3. __Implement high-speed SD card stability fix__
...
#### 4. __Configure the hardware module:__
...
1. __Edit the extlinux configuration file:__
