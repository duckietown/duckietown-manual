...
ROS implements the `publish-subscribe` pattern using __ROS Publishers__ and __ROS Subscribers__, and the `request-reply` pattern using __ROS Services__.
...
A Catkin package (also known as a _ROS package_) requires two files, `package.xml` and `CMakeLists.txt`.

1. Create `packages/my_package/package.xml` with the following content (replace `YOUR_FULL_NAME` and `YOUR_EMAIL@EXAMPLE.COM` accordingly):
