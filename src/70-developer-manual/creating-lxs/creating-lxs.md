...
1. The __LX recipe__, which is typically created by the instructor and hidden from the students.

2. The __LX repository itself__, which houses the notebooks and whatever code structure should be visible to the student.

An additional third repository is recommended, which has the same structure as the LX repository but contains
the __solutions__ and is kept private.
...
- [`dt-commons`](https://github.com/duckietown/dt-commons): A good choice
if the LX is pure Python and does not require ROS.

- [`dt-ros-commons`](https://github.com/duckietown/dt-ros-commons): A good
choice if you would like to use ROS in a standalone fashion (that is,
your exercise includes all required nodes).
...
- The `EXERCISE_NAME`: Replace `<LX_NAME_HERE>` with a title for your LX (__Note__: this should not contain spaces).
...
A file that you __must__ edit in order for your LX to work is the `.dtproject` file in the LX repo. In this file
...
- Change `<LX_NAME_HERE>` to the name of your LX.

- Change `<RECIPE_REPOSITORY_HERE>` to the name of the recipe repository that you created.

- Change `<RECIPE_BRANCH_HERE>` to the name of the branch that you want to use in your recipe repository.

- Change `<RECIPE_LOCATION_HERE>` to the path within the recipe repository where the recipe is stored. If you used the
template, you can leave this blank.
