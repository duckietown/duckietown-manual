...
Well‑written documentation determines whether code is _usable_ or _ignored_.

A brilliant package that saves developers weeks of effort is worthless if no‑one can discover or understand it.

Inline docstrings are especially powerful because they live next to the code they explain, are simple to update, and mirror the project’s structure. Tools such as __[Sphinx](https://www.sphinx-doc.org)__ then transform those docstrings into polished HTML pages.

```{note}
The book you are reading is _stand‑alone_ documentation. Inline documentation complements, rather than replaces, such higher‑level material.
```
...
The short and cheeky answer to this question is "everything, and in the right place". The long answer is the same, in other words: __everything that users may touch should be documented in its closest context.__
...
- Include a section with the arguments needed to initialize the node (the arguments of the `__init__` method), which will almost always be exactly the same as shown.

- Include a configuration section with the parameters for the node, their type, a short description, and their default value.

- Include an additional section describing the subscribers, publishers, and services, in this order.
...
- See [reStructuredText basics](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html).

- See [Quick references](https://docutils.sourceforge.io/docs/user/rst/quickref.html).

- See [Detailed specifications](https://docutils.sourceforge.io/docs/ref/rst/restructuredtext.html).
```
