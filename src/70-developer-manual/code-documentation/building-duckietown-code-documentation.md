...
Duckietown projects ship with a ready‑to‑use Jupyter Book skeleton under __`/docs/src`__. Refer to the [Book Writer Manual](book-devmanual-intro) for details on the supported features and a syntax cheat sheet.
...
`dts` launches a container, resolves dependencies, and writes the rendered HTML to __`/docs/html`__.
...
## Automatic API generation _(advanced)_
...
See the _Developers_ section of the [official Jupyter Book guide](https://jupyterbook.org/en/stable/advanced/developers.html) for detailed instructions.
