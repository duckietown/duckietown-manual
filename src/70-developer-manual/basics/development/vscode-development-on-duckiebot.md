...
- __Terminals:__ Use the integrated terminal for commands that must run inside the container.

- __ROS tools:__ If the container has ROS installed and is connected to the robot’s ROS network, tools like `rostopic` work from the terminal.

- __Persisting changes:__ Changes inside a container are automatically synced to your laptop.
