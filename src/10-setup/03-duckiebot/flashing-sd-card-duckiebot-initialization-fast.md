...
1. Use robot name: entebot+[version number], e.g., `entebot208`

2. Upload to AWS public / images S3 bucket

3. Upload to (Shared) Duckiebot Images GDrive

4. Create trackable links with Cuttly

5. Update redirects on Cloudflare > duckietown.com > Rules

6. update robot name in "default settings" paragraph below
-->
...
{{ dt_sd_card_not_written_trouble }}

{{ dt_sd_card_adapter_write_protection_trouble }}

{{ dt_sd_card_bad_archive_trouble }}

{{ dt_sd_card_token_setup_trouble }}
