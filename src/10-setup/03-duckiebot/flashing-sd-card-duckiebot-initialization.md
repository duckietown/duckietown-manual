...
```{attention}
__Updating from daffy to ente__: If you have a Duckiebot running the `daffy` version and want to update to `ente`, you must reflash a new SD card using the procedures below. The `dts duckiebot update` command does not currently support switching between `daffy` and `ente` versions. For information about updating within the same version, see [](ops-db-update).
```
