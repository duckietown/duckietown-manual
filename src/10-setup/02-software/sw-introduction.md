...
```{needget}
- A computer with at least `50 GB` of free space running [a supported OS](setup-computer).
...
```{attention}
If you are operating within a Duckietown Workspace, you can generally __skip all installation instructions__ in this section, but will still have to __do all the configuration steps__.
```
