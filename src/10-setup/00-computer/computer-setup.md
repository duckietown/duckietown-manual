...
If this is your first experience with Duckietown we _strongly recommend_ installing, either as dual boot or standalone installation, and using __Ubuntu__ by following these instructions: [](setup-install-ubuntu).

If you are a more experienced user, we support __Duckietown Workspaces on macOS and Windows__. Follow [](setup-devcontainer) for installation instructions.
...
If you are running Ubuntu in a VM (Virtual Machine), __make sure that your computer and Duckiebot appear as physical entities on the same network__. This is achieved by selecting the "bridged network adapter" (e.g., VirtualBox uses NAT by default), which will allow you to be on the same subnetwork as your Duckiebot.
